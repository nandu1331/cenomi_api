...
class RequiredFieldsCalculator:
    ...
    def calculate_required_fields(self, intent: IntentCategory, user_query: str, db_schema: str) -> List[str]:
        """
...
TEST_CASES = [
    {
        "intent": IntentCategory.TENANT_INSERT_STORE,
        "user_query": "I want to add a new store called 'Fashion Hub' in the mall",
        "expected_fields": ["name", "location", "category", "opening_date", "floor_number"]
    },
    {
        "intent": IntentCategory.TENANT_UPDATE_STORE,
        "user_query": "Change the location of store with ID 5",
        "expected_fields": ["store_id", "location"]
    },
    {
        "intent": IntentCategory.TENANT_DELETE_STORE,
        "user_query": "Remove the store with ID 10",
        "expected_fields": ["store_id"]
    },
    {
        "intent": IntentCategory.TENANT_INSERT_OFFER,
        "user_query": "Create a new 20% discount offer for the summer sale",
        "expected_fields": ["store_id", "title", "description", "discount_percentage", "start_date", "end_date"]
    },
]

def run_tests(calculator: RequiredFieldsCalculator):
    """Run test cases and print results"""
    results = []
    
    for i, test_case in enumerate(TEST_CASES):
        print(f"\n--- Running Test Case {i+1}: {test_case['intent'].value} ---")
        result = calculator.calculate_required_fields(
            test_case["intent"], 
        ...
        extra_fields = [field for field in result if field not in test_case["expected_fields"]]
        
        success = not missing_fields and not extra_fields
        ...
        print(f"Expected: {test_case['expected_fields']}")
    ...
    print(f"Passed: {successful_tests}/{len(TEST_CASES)} ({successful_tests/len(TEST_CASES)*100:.1f}%)")
    
    for r in results:
        status = "✅" if r["success"] else "❌"
        print(f"{status} Test {r['test_case']}: {r['intent']}")
...
def main():
    ...
    try:
        while True:
            ...
            result = calculator.calculate_required_fields(selected_intent, user_query, TEST_DB_SCHEMA)
