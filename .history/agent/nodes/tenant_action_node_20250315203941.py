    ...
    try:
        ...
        sql_generation_prompt_template = ChatPromptTemplate.from_messages([
            ("system", f"""
                ...
                Generate a valid SQL query to {operation_type} data in the '{{entity_type}}' table.
                Operation: {operation_type}. Entity: {entity_type}.
                Required Fields: {required_fields}.
                Tenant Data: {tenant_data}.
        
                **IMPORTANT: When generating the SQL query, refer to the values provided in the 'Tenant Data' dictionary for the field values.  For example, to use the 'offer_name', access it from the 'tenant_data' dictionary like this: tenant_data['offer_name'].**
                """),
            ("human", f"""
                Generate SQL query to {operation_type} {entity_type} using the following data.
                Tenant Data: {tenant_data}.
                Required fields: {required_fields}
                **Make sure to use the values from the 'Tenant Data' dictionary in your SQL query.**
                """)
        ])
