...
def handle_tenant_data_operation(state: AgentState, intent: IntentCategory) -> AgentState:
    ...
    try:
        ...
        user_query_string = "First key value pair are the unique fields, Use these to get the actual primary keys - "
        for field, value in tenant_data.items():
            user_query_string += f"{field}: '{str(value)}', "
        ...
        updated_state["next_node"] = "tool_selection_node" # Route to intent router
