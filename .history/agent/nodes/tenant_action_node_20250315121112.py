...
def handle_tenant_data_operation(state: AgentState, intent: IntentCategory) -> AgentState:
    ...
    if intent == IntentCategory.TENANT_UPDATE_OFFER:
        ...
        required_fields = ["offer_name", "discount_percentage"] # Example: Fields to update for offer

    elif intent == IntentCategory.TENANT_INSERT_OFFER:
        entity_type = "offer"
        operation_type = "insert"
        required_fields = ["offer_name", "description", "discount_percentage", "start_date", "end_date"] # Example: Fields to insert for offer

    elif intent == IntentCategory.TENANT_DELETE_OFFER:
        entity_type = "offer"
        operation_type = "delete"
        required_fields = ["offer_name"] # Example: Fields to delete offer (identify offer to delete)

    elif intent == IntentCategory.TENANT_UPDATE_STORE:
        entity_type = "store"
        operation_type = "update"
        required_fields = ["store_name", "operating_hours"] # Example for store - adjust as needed
    # ... (Add more entity/intent mappings as needed for stores, events, etc. - insert, delete, update) ...
    else:
        print(f"handle_tenant_data_operation - No entity/operation mapping for intent: {intent}. Defaulting to LLM call node.")
        state["next_node"] = "llm_call_node" # No handler found for intent
        return state
