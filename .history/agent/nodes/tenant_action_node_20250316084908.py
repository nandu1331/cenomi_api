...
from tools.sql_tenant_data_toolkit import SQLTenantDataToolkit  # This is our sql_database_query tool
...
def handle_tenant_data_operation(state: AgentState, intent: IntentCategory) -> AgentState:
    ...
    try:
        sql_toolkit = SQLTenantDataToolkit() # Initialize toolkit to get schema
        table_schema = sql_toolkit.get_table_schema(entity_type + "s") # Assuming table names are plural (offers, stores) - adjust if needed
        schema_info_dict = sql_toolkit.get_schema_info_dict() # Get full schema info dict

        if entity_type + "s" in schema_info_dict:
            table_schema_info = schema_info_dict[entity_type + "s"] # Access schema info for the specific table
            if operation_type == "insert":
                required_fields = [col['name'] for col in table_schema_info['columns'] if not col['nullable'] and not col['primary_key']] # Non-nullable, non-primary for insert
            elif operation_type in ["update", "delete"]:
                required_fields = [col['name'] for col in table_schema_info['columns'] if col['primary_key']] # Primary key for update/delete - for now, simplify to PK
            else:
                required_fields = [col['name'] for col in table_schema_info['columns']] # Default: all columns

            print(f"handle_tenant_data_operation - Fetched Schema for Entity: {entity_type}, Required Fields (from schema): {required_fields}")
        else:
            raise ValueError(f"Schema not found for entity type: {entity_type}")


    except Exception as e:
        error_message = f"Error fetching schema for entity {entity_type}: {e}"
        print(error_message)
        state["agent_response"] = error_message # Set error response
        state["next_node"] = "output_node"
        return state
