...
def handle_tenant_data_operation(state: AgentState, intent: IntentCategory) -> AgentState:
    ...
    try:
        ...
        sql_generation_prompt_template = ChatPromptTemplate.from_messages([
            ("system", f"""
                You are a SQL query generator for tenant data operations in a mall database.

                *Database Schema:*
                {db_schema}  -- Provide the database schema here (e.g., CREATE TABLE statement for 'offers' table)

                *Task:* Generate a valid SQL query to perform a *{operation_type}* operation on the *{entity_type}* table using the provided *Tenant Data*.

                *Tenant Data:*
                {tenant_data} -- Data for the operation (e.g., dictionary or string representation of data)

                - Use the provided Tenant Data to populate the SQL query.
                - Ensure the generated SQL query is valid and executable against the given Database Schema.
                """),
            ("human", f"""
                Generate SQL query for {operation_type} operation on {entity_type} table.
