...
def generate_sql_query(user_query: str, intent: IntentCategory, conversation_history: str) -> str:
    ...
    try:
        sql_query = sql_query_generation_chain.invoke({"db_schema_info": db_schema_str, "user_query": user_query, "conversation_history": conversation_history})
    ...
    for tool_name_str in selected_tool_names:
        ...
        if tool_name == ToolName.VECTOR_DB_SEARCH:
            ...
            if dynamic_sql_query: # Check if SQL query was generated successfully
                ...
                tool_outputs[ToolName.SQL_DB_SEARCH.value] = "Error: Could not generate SQL query." # Error message if SQL generation failed
