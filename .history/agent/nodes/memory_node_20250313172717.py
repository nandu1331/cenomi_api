...
buffer_memory = ConversationBufferMemory(memory_key="buffer_history", input_key="user_query", output_key="response")
...
def memory_node(state):
    ...
    buffer_memory.chat_memory.messages = buffer_memory.load_memory_variables({})
    buffer_memory.save_context({"user_query": user_query}, {"response": None})
    updated_buffer_history_dict = buffer_memory.load_memory_variables({})
    updated_buffer_history = updated_buffer_history_dict.get("buffer_history", [])
    
    current_summary = state.get("summary_history", "") or ""
