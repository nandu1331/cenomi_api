...
def _analyze_query_specificity(query: str, intent: IntentCategory) -> Dict[str, bool]:
    ...
    specific_intents = [
        ...
        IntentCategory.CUSTOMER_QUERY_EVENT_INFO, IntentCategory.CUSTOMER_QUERY_SERVICE_QUERY
    ]
    ...
    specificity_indicators = [
        has_list_keyword, has_filter_keyword, has_specific_entity, has_data_operation,
        has_attribute_keyword, has_comparison, has_structured_pattern, has_quantity_pattern,
        has_named_entity, has_proper_noun, intent_suggests_specific
    ]
    vagueness_indicators = [has_vague_keyword, has_conceptual_keyword]
    ...
    return {"is_specific": is_specific, "specificity_score": final_specificity_score}
...
def _get_llm_specificity_score(query: str) -> int:
    specificity_prompt = ChatPromptTemplate.from_messages([
        ("system",
            ...
            Analyze if the query is specific (concrete, well-defined) or vague (abstract, general).
    ...
    try:
        ...
        score = int(specificity_chain.invoke({"query": query}).strip())
